...
TEST(Vector_View, dynamic_of_static)
{
  ...
  expr(v, _assign_, 1);
  auto view_v = create_view_subvector(v, 1, 3);
...
TEST(Vector_View, static_of_static)
{
  Tiny_Vector<int, 10> v;
  expr(v, _assign_, 1);
  auto view_v = create_view_subvector(v, std::integral_constant<std::size_t, 1>(),
...
TEST(Vector_View, constness_check)
{
  ...
  expr(v, _assign_, 1);
  v[1]        = 2;
  auto view_v = create_view_subvector(v.as_const(), 1, 3);
